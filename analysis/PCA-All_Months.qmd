...
convert_to_avg <- function(col) {
   ...
   for (cell in col) {
      ...
      if (any(str_detect(cell, "\\d+ \\(.*\\)"), na.rm = T)) {
        ...
        cell = mean(num)
        print(cell)
      #.  } else if (any(str_detect(cell, "\\d+\\.\\d+"), na.rm = T)) {
      #   print(1)
      #   num = as.double(unlist(str_extract_all(cell, "\\d+\\.\\d+")))
      #   cell = mean(num)
      #   print(cell)
...
want_Site <- c("SG")
#want_Site <- c("DG1", "LAG1", "LAG2", "OL", "SG")
...
width <- 10000
height <- width / 16 * 10
jpeg("Rplot01.jpg", res = 700, width = width, height = height)

p_segment + vars_p + plot_layout(widths = c(3, 1), heights = c(5, 1), guides = "collect") & theme(legend.position = 'right')
...
AAAAAAACCC
"
(p_segment | (vars_p / guide_area())) + plot_layout(guides = "collect")
```

Color = Month or Shape = Month?

Grey scale?

individual site with arrows per image + 0 arrow image

try to match color

may an inset -\> with a box around the inset
