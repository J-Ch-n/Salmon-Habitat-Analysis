...
convert_to_avg <- function(col) {
   ...
   for (cell in col) {
      ...
      if (any(str_detect(cell, "\\d+ \\(.*\\)"), na.rm = T)) {
        ...
        num = as.double(unlist(str_extract_all(cell, "\\d+\\.\\d+, \\d+\\.\\d+")))
        cell = mean(num)
   }
...
safe_colorblind_palette <- c("#88CCEE", "#CC6677", "#DDCC77", "#117733", "#332288", "#AA4499", 
                             "#44AA99", "#999933", "#882255", "#661100", "#6699CC", "#888888")
pc_component <- c("DO", "Velocity", "RCT Depth", "Tempearture", "Volume")
graph_pca <- clean_habitat
graph_pca$pc1 <- all_month_pca$x[,1]
graph_pca$pc2 <- all_month_pca$x[,2]
...
p_segment + vars_p + plot_layout(widths = c(3, 1), heights = c(5, 1), guides = "collect") & theme(legend.position = 'bottom')
