...
for (want_Site in all_site) {
  ...
  p_segment <- ggplot(data = graph_pca, 
    ...
    geom_segment(
               ...
               y = pc2,
               yend = get_arrow_ycoordinates(Site, SiteName, Month, graph_pca, want_Site),
               shape = as.factor(Month))) + ## add a function that outputs a matrix of   coordinates with input of the data coordinate.
    scale_color_manual(values = safe_colorblind_palette) +
    coord_equal() + 
    xlab("PC 1") +
    ylab("PC 2") +
...
for (want_Site in all_site) {
  ...
  p_segment <- ggplot(data = graph_pca, 
    ...
    geom_segment(
               ...
               yend = get_arrow_ycoordinates(Site, SiteName, Month, graph_pca, want_Site),
               shape = as.factor(Month)),
               ) + ## add a function that outputs a matrix of   coordinates with input of the data coordinate.
    scale_color_manual(values = safe_colorblind_palette) +
    coord_equal() + 
    xlab("PC 1") +
    ylab("PC 2") +
